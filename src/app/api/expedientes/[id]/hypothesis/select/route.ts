...
export async function POST(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const updated = await prisma.investigation.update({ where: { id: investigation.id }, data: { selectedHypothesisId: h.id } });
  return NextResponse.json({ ok: true, selectedHypothesisId: updated.selectedHypothesisId, completed: updated.status === 'COMPLETED', correct: updated.status === 'COMPLETED' ? h.isCorrect : null });
}
