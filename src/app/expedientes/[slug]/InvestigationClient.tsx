...
const isTabUnlocked=(x:string)=>{
 ...
 return <main className="casePage">{message&&<div className="message">{message}</div>}<header className="caseHero"><nav className="nav" aria-label="Navegación de investigación">
...
{tab==='Hipótesis'&&<div className="narrativeGrid">{narrative.hypotheses.map(x=>{const selected=narrative.conclusion?.selectedHypothesisId===x.id;return <article className={`card ${selected?'found':''}`} key={x.id}><span className="tag">{x.code}</span><h3>{x.title}</h3><p>{x.description}</p>{selected&&<div className="foundMark">✓ Hipótesis seleccionada</div>}<button className="btn" onClick={()=>chooseHypothesis(x.id)} disabled={busy||selected||status==='COMPLETED'}>{selected?'Hipótesis seleccionada':status==='COMPLETED'?'Investigación cerrada':'Elegir esta hipótesis'}/button></article>})}</div>}
